  ...
  (let ((operand-v (codegen (operand expression))))
    (when operand-v
      (let ((f (llvm:named-function *module*
                                    (format nil "unary~a"
                                            (opcode expression)))))
        (unless (not (wrapper-null-p f))
          (error 'kaleidoscope-error :message "Unknown unary operator"))
        (llvm:build-call *builder* f (vector operand-v) "unop")))))
      ...
      (case (operator expression)
        ...
        (otherwise
         (let ((f (llvm:named-function *module*
                                       (format nil "binary~a"
                                               (operator expression)))))
           (assert (not (wrapper-null-p f)) ()
                   "binary operator not found!")
           (llvm:build-call *builder* f (vector l r) "binop")))))))
...
(defmethod codegen ((expression if-expression))
  (let ((cond-v (codegen (condition expression))))
    (when cond-v
      (setf cond-v
            (llvm:build-f-cmp *builder* 
                              :/= cond-v (llvm:const-real (llvm:double-type) 0d0)
                              "ifcond"))
              ...
              (when else-v
                ...
                (let ((pn (llvm:build-phi *builder*
                  ...
                  (llvm:add-incoming pn
                                     (vector then-v else-v)
                                     (vector then-bb else-bb))
                  pn)))))))))
    ...
    (when start-val
      ...
      (let* ((preheader-bb (llvm:insertion-block *builder*))
        ...
        (let ((variable (llvm:build-phi *builder*
                                        (llvm:double-type)
                                        (var-name expression))))
          (llvm:add-incoming variable (vector start-val) (vector preheader-bb))
          (let ((old-val (gethash (var-name expression) *named-values*)))
            ...
            (when (codegen (body expression))
              (let ((step-val (if (step* expression)
                                  ...
                                  (llvm:const-real (llvm:double-type) 1d0))))
                (when step-val
                    ...
                    (when end-cond
                      ...
                      (let ((loop-end-bb (llvm:insertion-block *builder*))
                        ...
                        (llvm:add-incoming variable
                                           (vector next-var) (vector loop-end-bb))
                        (if old-val
